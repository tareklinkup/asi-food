					...
					<div class="row">
						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right"> Com. </label>
									<div class="col-sm-4 no-padding-right" style="display: flex">
										<input type="number"  id="commission" step="0.01"  v-model.number="selectedProduct.commission_percent" v-on:input="commissionTotal" class="form-control" /> 
										<span style="display: inline-block; margin-left: 4px; font-size: 15px">%</span>
									</div>
									<div class="col-sm-5">
										<input type="number" v-model="commission" step="0.01" v-on:input="commissionTotal" class="form-control" />
									</div>
								</div>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr class="">
							...
							<th style="width:8%;color:#000;">Com. (%)</th>
									...
									<tr>
										<td>
											<div class="form-group">
												...
												<div class="col-sm-12">
													<select class="form-control" v-model="sales.payment_type" style="padding: 0 6px;" required>
	...
	new Vue({
		...
		methods:{
			...
			getProducts(){
				...
				axios.post('/get_products', {isService: this.sales.isService, categoryId: category_id}).then(res=>{
					if(this.sales.salesType == 'wholesale'){
						this.products = res.data.filter((product) => product.Product_WholesaleRate > 0);
						this.products.map((product) => {
							product.commission_percent = 0.00;
			...
			commissionTotal(){
				if(event.target.id == 'commission'){
					this.commission  = ((parseFloat(this.selectedProduct.total) * parseFloat(this.selectedProduct.commission_percent)) / 100).toFixed(2);
				} else {
					this.selectedProduct.commission_percent = (parseFloat(this.commission ) / parseFloat(this.selectedProduct.total) * 100).toFixed(2);
				}
			},
			productTotal(){
			...
			addToCart(){
				...
				let product = {
					...
					commission_percent: this.selectedProduct.commission_percent,
				}
