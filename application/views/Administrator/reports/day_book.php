					...
					<div style="width:50%;border:1px solid black;border-left:none;position:relative;">
						<table class="day-book-table">
							...
							<tbody>
								...
								<template v-if="employeeOldPayments.length > 0">
									<tr>
										<td class="sub-heading">Employee Payments (old)</td>
										<td class="sub-value">{{ totalEmployeeOldPayments }}</td>
									</tr>
									<tr v-for="payment in employeeOldPayments">
										<td>{{ payment.Employee_Name }}</td>
										<td>{{ payment.totalAmount | decimal }}</td>
									</tr>
								</template>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalEmployeeOldPayments(){
				return this.employeeOldPayments.reduce((prev, curr) => {
					return prev + parseFloat(curr.totalAmount)
				}, 0).toFixed(2);
			},
			totalInitialLoan(){
			...
			totalOut(){
				return parseFloat(this.totalPurchase) +
					...
					parseFloat(this.totalEmployeeOldPayments) +
		...
		methods: {
			...
			getEmployeeOldPayments(){
				...
				.then(res => { 
					...
					this.employeeOldPayments = payments;
