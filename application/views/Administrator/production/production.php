                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <form id="materialForm" v-on:submit.prevent="addToCart">
                                    <div class="form-group clearfix" style="margin-bottom:2px">
                                        <label class="col-sm-4 control-label no-padding-right"> Category </label>
                                        <div class="col-sm-8">
                                            <v-select label="name" v-bind:options="materialCategories" v-model="selectedMaterialCategory"></v-select>
                                        </div>
                                    </div>
...
<script>
    ...
    new Vue({
        ...
        watch: {
            "selectedProduct.quantity": function (quantity) {
                if (quantity && this.production.total_cost) {
                    let finishProductPrice = this.production.total_cost / quantity;
                    this.selectedProduct.Product_Purchase_Rate = finishProductPrice;
                }
            },
        ...
        methods: {
            ...
            getMaterials() {
                ...
                if (this.selectedMaterialCategory) {
                    category_id = this.selectedMaterialCategory.id;
                }

                axios.post('/get_materials', {category_id})
                    .then(res => {
