...
// ) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;
    ...
    public function assetAccount() {
        ...
        $data['content'] = $this->load->view('Administrator/asset_account', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function getAssetAccounts() {
        $categories = $this->db->query("
            SELECT * FROM tbl_asset_account WHERE status = 'a' AND branchid =? 
        ", $this->session->userdata('BRANCHid'))->result();

        echo json_encode($categories);
    }

    public function saveAssetAccount() {
        ...
        try {
            ...
            $insert = $this->db->insert('tbl_asset_account', $data);
    ...
    public function updateAssetAccount() {
        $res = new stdClass;
        try {
           $obj = json_decode($this->input->raw_input_stream);
           ...
           $data = array(
            ...
            $result = $this->db->where('id', $obj->id)->update('tbl_asset_account', $data);
            if ($result) {
                $res->message = 'Updated successfully !';
            }
           
        } catch (\Exception $e) {
           $res->message = "Updated failed !". $e->getMessage();
        }

        echo json_encode($res);
    }

    public function deletedAssetAccount() {
        $res = new stdClass;
        ...
        try {
            ...
            $res->message = "Deleted failed !". $e->getMessage();
        }
    ...
    public function assetItem() {
        ...
        $data['title'] = "Add Asset";
        $data['content'] = $this->load->view('Administrator/asset_item', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function getAssetItems() {
        $items = $this->db->query("SELECT * FROM tbl_asset_item WHERE status ='a' AND branchid = ?", $this->session->userdata('BRANCHid'))->result();
        echo json_encode($items);
    }

    public function saveAssetItem() {
        $obj = json_decode($this->input->raw_input_stream);
        $res = new stdClass;
        try {
            $data = array(
            ...
            $insert = $this->db->insert('tbl_asset_item', $data);
    ...
    public function updateAssetItem() {
        $res = new stdClass;
        try {
           $obj = json_decode($this->input->raw_input_stream);
           ...
           $data = array(
            ...
            $result = $this->db->where('id', $obj->id)->update('tbl_asset_item', $data);
            if ($result) {
                $res->message = 'Updated successfully !';
            }
    ...
    public function deleteAssetItem() {
        $res = new stdClass;
