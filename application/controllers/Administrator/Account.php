...
class Account extends CI_Controller {
    ...
    public function cashTransactionInvoice($id) {
		...
		$data['transaction'] = $this->db->query("
            select ct.*, acc.* from tbl_cashtransaction as ct
            join tbl_account as acc on acc.Acc_SLNo = ct.Acc_SlID
            where ct.status != 'd' and ct.Tr_SlNo = {$id}
        ")->row();
        $data['content'] = $this->load->view('Administrator/account/cash_transaction_invoice', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getCashLedger() {
        ...
        $ledger = $this->db->query("
            ...
            select 
                ...
                ep.payment_date as date,
                concat('Employee salary (old) - ', m.month_name, ' - ', e.Employee_Name) as description,
                0.00 as in_amount,
