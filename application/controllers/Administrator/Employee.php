...
class Employee extends CI_Controller
{
    ...
    public function checkPaymentMonth()
    {
        ...
        $query = $this->db->query("SELECT month_id FROM tbl_employee_payment_new WHERE month_id = ? and branch_id = ? and status = 'a'",[$monthId, $this->session->userdata("BRANCHid")]);
        if($query->num_rows() > 0 ) {
    ...
    public function getSalaryDetails()
    {
        ...
        $payments = $this->db->query(
            ...
            from tbl_employee_payment_details pd
            join tbl_employee_payment_new ep on ep.id = pd.payment_id
            join tbl_month m on m.month_id = ep.month_id
            join tbl_employee e on e.Employee_SlNo = pd.employee_id
            left join tbl_designation de on de.Designation_SlNo = e.Designation_ID
            left join tbl_department d on d.Department_SlNo = e.Department_ID
    ...
    public function saveSalaryPayment()
    {
        ...
        try {
            ...
            $this->db->insert('tbl_employee_payment_new', $payment);
            ...
            $this->db->where('id', $payment_id)->update('tbl_employee_payment_new', ['total_payment_amount' => $total_payment_amount]);
    ...
    public function updateSalaryPayment()
    {
        ...
        try {
            ...
            $this->db->update('tbl_employee_payment_new', $payment);
    ...
    public function deletePayment()
    {
        ...
        try{
            ...
            $this->db->set(['status'=>'d'])->where('id', $data->paymentId)->update('tbl_employee_payment_new');
