...
class Purchase extends CI_Controller
{
    ...
    public function getMaterialPurchaseDetails(){
        ...
        $purchaseDetails = $this->db->query("
            ...
            where pd.status = 'a' 
            and purchase_id = '$options->purchase_id'
    ...
    public function addMaterialPurchase(){
        ...
        try{
            ...
            foreach($data->purchasedMaterials as $purchasedMaterial){
                ...
                $this->updateMaterialAveragePurchasePrice($purchasedMaterial->material_id);
            }
    ...
    private function updateMaterialAveragePurchasePrice($material_id)
    {
        $previous = $this->db->query("
            SELECT 
                ifnull(SUM(total), 0) as total_purchase_rate,
                ifnull(SUM(quantity), 0) as total_purchase_qty 
            FROM tbl_material_purchase_details 
            WHERE material_id = $material_id AND status = 'a'
            and branch_id = '$this->brunch'
        ")->row();

        $averagePurchaseRate = $previous->total_purchase_rate / $previous->total_purchase_qty;
        $this->db->where('material_id', $material_id)->update('tbl_materials', ['purchase_rate' => $averagePurchaseRate]);
    }
