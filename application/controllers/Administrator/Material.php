...
class Material extends CI_Controller{
    ...
    public function getMaterials(){
        ...
        if (isset($data->category_id) && !empty($data->category_id)) {
            $clauses .= " and m.category_id = '$data->category_id'";
        }

        if(isset($data->status) && $data->status != ''){
            $clauses .= " and m.status = '$data->status'";
        }else{
            $clauses .= " and m.status = 1";
        }
    ...
    public function updateMaterial(){
        ...
        try{
            ...
            $nameQuery = $this->db->query("select * from tbl_materials where name = '$data->name' and branch_id = '$this->brunch' and material_id != '$data->material_id'");
            $nameCount = $nameQuery->num_rows();
            if($nameCount != 0){
                $res = ['success'=>false, 'message'=>'Duplicate material name ' . $data->name];
                echo json_encode($res);
    ...
    public function getMaterialStock(){
        ...
        $stock = $this->db->query("
            ...
            from tbl_materials m
            join tbl_material_categories pc on pc.id = m.category_id
            join tbl_unit u on u.Unit_SlNo = m.unit_id
            where m.branch_id = '$this->brunch'
            $materialClause
        ")->result();
    ...
    public function getMaterialTotalStock(){
        ...
        $stock = $this->db->query("
            select
                ...
                (select m.purchase_rate * stock_quantity) as stock_value
            from tbl_materials m
            left join tbl_material_categories pc on pc.id = m.category_id
